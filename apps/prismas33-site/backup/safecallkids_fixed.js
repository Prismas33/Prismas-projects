...
if (typeof firebase !== 'undefined') {
    try {
        ...
        // Sign in anonymously to avoid CORS issues
        auth.signInAnonymously().then(() => {
            isFirebaseReady = true;
            console.log('Firebase authenticated successfully');
        }).catch((error) => {
            console.warn('Firebase auth error (continuing without auth):', error);
            isFirebaseReady = true; // Continue even if auth fails
        });
    } catch (error) {
...
class SafeCallKids {
    ...
    async handleFileUpload(file) {
        if (!storage || !isFirebaseReady) {
            this.showNotification(
                this.currentLanguage === 'pt' 
                    ? 'Firebase não está pronto. Aguarde um momento e tente novamente.' 
        ...
        try {
            ...
            // Try different storage reference approaches
            let storageRef;
            try {
                storageRef = storage.ref().child(`apks/${fileName}`);
            } catch (error) {
                console.warn('Using alternative storage ref method');
                storageRef = storage.ref(`apks/${fileName}`);
            }
            ...
            this.showNotification(
                this.currentLanguage === 'pt' 
                    ? 'Erro ao inicializar upload. Verifique a conexão.' 
                    : 'Error initializing upload. Check your connection.'
            );
            this.resetModalState();
        }
    }
