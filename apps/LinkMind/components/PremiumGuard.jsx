...
export default function PremiumGuard({ children, showMessage = true }) {
  ...
  if (!user) {    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md mx-auto text-center">          <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('configuracoes.premiumGuard.acessoRestrito')}</h2>
          <p className="text-gray-600 mb-6">{t('configuracoes.premiumGuard.precisaLogin')}</p>
          <Link href="/login">
            <button className="bg-[#7B4BFF] hover:bg-[#6A3EEE] text-white px-6 py-3 rounded-lg font-medium transition-colors">
              {t('configuracoes.premiumGuard.fazerLogin')}
            </button>
          </Link>
        </div>
      </div>
  ...
  if (!accessStatus?.hasAccess) {
    ...
    return (      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-4">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md mx-auto text-center">
          ...
          </div>          <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('configuracoes.premiumGuard.acessoPremiumNecessario')}</h2>
          <p className="text-gray-600 mb-6">
            {accessStatus?.type === 'expired' 
              ? t('configuracoes.premiumGuard.trialExpirou')
              : t('configuracoes.premiumGuard.funcionalidadePremium')}
          </p>
          <div className="space-y-3">
            <Link href="/subscription">
              <button className="w-full bg-[#7B4BFF] hover:bg-[#6A3EEE] text-white px-6 py-3 rounded-lg font-medium transition-colors">
                {t('configuracoes.premiumGuard.verPlanos')}
              </button>
            </Link>
            <Link href="/dashboard">
              <button className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-3 rounded-lg font-medium transition-colors">
                {t('configuracoes.premiumGuard.voltarDashboard')}
              </button>
            </Link>
          </div>
        </div>
